...
+ (id)gradientWithBeginningColor:(CGColorRef)begin endingColor:(CGColorRef)end
  ...
  CTGradientElement color2;
	...
	float *components = (float *)CGColorGetComponents(begin);
	color1.red = components[0];
	color1.green = components[1];
	color1.blue = components[2];
	color1.alpha = components[3];
	
	components = (float *)CGColorGetComponents(end);
	color2.red = components[0];
	color2.green = components[1];
	color2.blue = components[2];
	color2.alpha = components[3];
...
- (void)fillRect:(NSRect)rect angle:(float)angle
  ...
  CGContextRef currentContext = UICurrentContext();
